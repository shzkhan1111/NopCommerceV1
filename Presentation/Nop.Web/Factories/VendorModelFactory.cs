...
namespace Nop.Web.Factories
{
    ...
    public partial class VendorModelFactory : IVendorModelFactory
    {
        ...
        public virtual async Task<List<ForwardDetails>> PrepareVendorListByCustomerQuoteAsync(int customerQuoteId)
        {
            if (customerQuoteId == 0)
                throw new ArgumentNullException();
            var fwdList = new List<ForwardDetails>();
            var vendors = await _vendorService.GetVendorByCustomerQuoteIdAsync(customerQuoteId);
            if (vendors != null && vendors.Count > 0)
                fwdList = vendors.Select(x => new ForwardDetails()
                {
                    Name = x.Name,
                    Email = x.Email,
                    Id = x.Id
        ...
        public virtual async Task<List<ForwardDetails>> PrepareVendorChatListByCustomerQuoteAsync(int customerQuoteId)
        {
            if (customerQuoteId == 0)
                throw new ArgumentNullException();
            var fwdList = new List<ForwardDetails>();
            var vendorQuote = await _quoteService.GetVendorQuoteByIdAsync(customerQuoteId);
            var vendors = await _vendorService.GetVendorByCustomerQuoteIdAsync(vendorQuote.CustomerQuoteId);
            if (vendors != null && vendors.Count > 0)
                fwdList = vendors.Select(x => new ForwardDetails()
                {
                    Name = x.Name,
