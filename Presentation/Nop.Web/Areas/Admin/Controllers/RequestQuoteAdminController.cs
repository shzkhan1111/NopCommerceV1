...
{
    public partial class RequestQuoteAdminController : BaseAdminController
    {
        #region Fields
        ...
        public RequestQuoteAdminController(ICustomerService customerService,
        ...
        public virtual async Task<IActionResult> DeleteCustomerQuoteItem(int quoteItemId)
        {
            try
            {
                _quoteModelFactory.DeleteCustomerQuoteItem(quoteItemId);
        ...
        public virtual async Task<IActionResult> RestoreCustomerQuoteItemId(int quoteItemId)
        {
            try
            {
                _quoteModelFactory.RestoreCustomerQuoteItem(quoteItemId);
                //var quoteItem = await _quoteService.GetCustomerQuoteItemByIdAsync(quoteItemId);
        ...
        List<QuoteAttribueMappingModel> GetAttributeList(int[] mappingIds, int[] attributesIds, string[] nameList, string[] valueList)
        {
            List<QuoteAttribueMappingModel> list = new List<QuoteAttribueMappingModel>();

            for (int i = 0; i < mappingIds.Count(); i++)
            {
                QuoteAttribueMappingModel model = new QuoteAttribueMappingModel();
                model.Id = mappingIds[i];
                model.AttributeId = attributesIds[i];
                model.Name = nameList[i];
                model.Value = valueList[i];
        ...
        public virtual async Task<IActionResult> AddAttributesToQuote(int [] mappingIds, int[] attributesIds, string[] nameList, string[] valueList, int QuoteId)
        {
            try
            {
                List<QuoteAttribueMappingModel> attributeList = GetAttributeList(mappingIds, attributesIds, nameList, valueList);
                await _quoteModelFactory.AddAttributesToQuote(attributeList, QuoteId);
                return Json(new
                {
                    success = true,
